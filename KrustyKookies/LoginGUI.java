...
public class LoginGUI implements Observer{
	...
	public void update(Observable o, Object arg) {
		...
		if(!trutru){
			textField.setText("");
			bottomText.setText("incorrect password");
			
		} else {
			String passwrd = CurrentUser.instance().getCurrentUserId();
			if(passwrd.compareToIgnoreCase("barcode")==0){
				SearchGUI bc1 = new SearchGUI(db);
			} else if (passwrd.compareToIgnoreCase("production")==0){
				ProductionGUI k2 = new ProductionGUI(db);
			// For customers
			} else {
